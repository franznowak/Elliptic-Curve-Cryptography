...
class Point(object):
    ...
    def negate(self):
        ...
        # This is an optional extension and is not evaluated
    ...
    def double(self):
        ...
        if self.y % self.curve.p == -self.y % self.curve.p:
            return PointInf(self.curve)
        l = (3*self.x**2 + self.curve.a) * mod_inv(2*self.y, self.curve.p) % self.curve.p
        x_result = (l**2 - 2*self.x) % self.curve.p
        y_result = (l*(self.x - x_result) - self.y) % self.curve.p
        return Point(self.curve, x_result, y_result)
    ...
    def add(self, other):
        ...
        # Adding the identity element just results in the point. For the actual addition, we need to check if the result is the neutral element by checking if they ys together add up to 0 mod p.
        if self.is_equal(other):
            return self.double()
        elif isinstance(other, PointInf):
            return Point(self.curve, self.x, self.y)
        elif self.y % self.curve.p == -other.y % other.curve.p:
            return PointInf(self.curve)
        else:
            l = (other.y - self.y) * mod_inv(other.x - self.x, self.curve.p) % self.curve.p
            x_result = (l**2 - self.x - other.x) % self.curve.p
            y_result = (l*(self.x - x_result) - self.y) % self.curve.p
            return Point(self.curve, x_result, y_result)
    ...
    def scalar_multiply(self, scalar):
        ...
        if not isinstance(scalar, int):
            raise ValueError("invalid scalar, has to be int or long")
        scalar_2 = format(scalar, "b")
        result = PointInf(self.curve)
        for i in range(len(scalar_2)):
...
def KeyGen(params):
    ...
    x = 0
    n_bits = len(format(params.q, "b"))
    while x == 0 or x >= params.q:
...
def Sign(params, x, msg):
    ...
    k = 0
    n_bits = len(format(params.q, "b"))
    while k == 0 or k >= params.q:
        k = secrets.randbits(n_bits)
    while True:
